...
NODE
  ...
  if [[ "$(defaults read -g com.apple.keyboard.fnState 2>/dev/null)" == "0" ]]; then
    add_check pass "F1/F2 preference" "stored preference uses media keys; verify the live Function Keys switch and physical keys"
  else
    add_check fail "F1/F2 preference" "disable standard function keys in Keyboard settings"
  fi
