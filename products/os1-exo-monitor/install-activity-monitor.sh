...
finish() {
  ...
  if [[ "$TASK_COMMITTED" -ne 1 && "$TASK_MUTATED" -eq 1 ]]; then
    # File custody begins before the first write, independently of whether
    # launchd has been touched. EXIT also covers explicit validation failures.
...
if [[ "$(basename "$TASK_BASE_EXECUTABLE")" == "run-air.sh" ]]; then
  ...
  # Stage both inputs before touching Python startup. Backups remain in the
  # recovery directory even when a copy, plist edit, or explicit check fails.
...
if [[ "$TASK_SOURCE_MODE" -eq 1 ]]; then
  # run-air.sh owns EXO_DASHBOARD_DIR. The overlay applies this dedicated
  # setting before importing EXO, without rewriting that existing wrapper.
