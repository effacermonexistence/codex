...
import {
  exportLearningRows, LEARNING_HALF_LIFE_MS, routeSeed, updateLearning, validLearningObservation,
  type LearningObservation,
} from "../src/route-learning";
import { supportsCompletionFeedback, supportsRouteLearning } from "../src/capabilities";

vi.mock("cloudflare:workers", () => ({ DurableObject: class {} }));
import service from "../src/index";

const adopted: LearningObservation = { provider: "codex", model: "gpt-6-sol", effort: "medium",
  task_class: "executed_change", adopted: true, duration_ms: 40_000 };
...
describe("route learning capability", () => {
  ...
  it("is offered only by the adapter pinned for this policy", async () => {
    ...
    expect(await supportsRouteLearning(binding({ ...v37, route_learning_schema: 2 }), policy, 0)).toBe(false);
