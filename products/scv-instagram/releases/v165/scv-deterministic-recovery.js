...
function routeAwareRecoverySurfaceText(input = {}, originalPlan = {}) {
  const text = routeAwareRecoveryText(input, originalPlan)
  // v165: these are an owner-approved fixed checkpoint surface, not model
  // prose. Preserve their exact punctuation; the generic surface normalizer
  // below continues to govern every other recovery line.
  if (
    String(originalPlan?.action || '') === 'await_double_check_confirmation' &&
    CHECKPOINT_NEUTRAL_NUDGES.includes(text)
  ) return text
  return text
    .replace(/,/g, '')
    .replace(/\.(?=\s|$)/g, '')
